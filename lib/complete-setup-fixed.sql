...
CREATE POLICY "Traders can update assigned orders" ON public.orders
  ...
  WITH CHECK (
    ...
    AND (
      ...
      (SELECT status FROM public.orders WHERE id = id) = status 
      OR (id = id AND buyer_id = buyer_id AND trader_id = trader_id 
          AND transporter_id = transporter_id AND title = title 
          AND description = description AND amount = amount 
          AND shipping_address = shipping_address AND billing_address = billing_address)
    )
  );
...
CREATE POLICY "Transporters can update assigned orders" ON public.orders
  ...
  WITH CHECK (
    ...
    AND (
      ...
      (id = id AND buyer_id = buyer_id AND trader_id = trader_id 
       AND title = title AND description = description 
       AND amount = amount AND shipping_address = shipping_address)
    )
  );
